...
$stmt = $pdo->prepare("SELECT u.* FROM follows f JOIN users u ON f.follower_id = u.id WHERE f.following_id = ?");
$stmt->execute([$view_id]);
$followers = $stmt->fetchAll();
?>

<div class="container mx-auto px-6 py-12 max-w-3xl">
    <h2 class="text-2xl font-bold mb-8">Pengikut @<?= htmlspecialchars($user['username']) ?></h2>
    <?php if (!$followers): ?>
        <div class="p-4 bg-gray-100 rounded">Belum ada followers.</div>
    <?php else: ?>
        <div class="space-y-4">
